...
export const createConsentModal = (api, createMainContainer) => {
    ...
    if (!dom._cmContainer) {
         ...
         * Can be controlled via guiOptions.consentModal.showCloseButton
         */
        const showCloseButton = !consentModalOptions || consentModalOptions.showCloseButton !== false;
        
        if (consentModalTitleValue && closeIconLabelData && isBoxLayout && showCloseButton) {
            ...
            // Show the button
            if (dom._cmCloseIconBtn.style.display === 'none') {
                dom._cmCloseIconBtn.style.display = '';
            }
        } else if (dom._cmCloseIconBtn) {
            // Hide the button if it exists but shouldn't be shown
            dom._cmCloseIconBtn.style.display = 'none';
        }

        // Add logo support
        const showLogo = consentModalOptions && consentModalOptions.showLogo;
        const logoUrl = consentModalOptions && consentModalOptions.logoUrl;
        if (showLogo && !dom._cmLogo) {
            const baseUrl = globalObj._baseUrl || '';
            const pngPath = logoUrl || (baseUrl ? `${baseUrl}/cc-logo.png` : 'cc-logo.png');
            const svgPath = logoUrl ? logoUrl.replace('.png', '.svg') : (baseUrl ? `${baseUrl}/cc-logo.svg` : 'cc-logo.svg');
            
            dom._cmLogo = createNode('img');
            addClassCm(dom._cmLogo, 'logo');
            setAttribute(dom._cmLogo, 'alt', 'Logo');
            
            // Try PNG first, fallback to SVG on error (faster than fetch HEAD)
            dom._cmLogo.onerror = function() {
                this.onerror = null; // prevent infinite loop
                this.src = svgPath;
            };
            setAttribute(dom._cmLogo, 'src', pngPath);
            
            appendChild(dom._cmBody, dom._cmLogo);
        }
    ...
    if (!dom._cmBtnGroup) {
        ...
        // Add Show Preferences button in the middle
        if (dom._cmShowPreferencesBtn) {
            appendChild(dom._cmBtnGroup, dom._cmShowPreferencesBtn);
            
            // Add uneven class if not all accept buttons exist
        ...
        (acceptAllBtnData || acceptNecessaryBtnData || dom._cmShowPreferencesBtn) && appendChild(dom._cmBody, dom._cmBtnGroup);
    ...
    if (footerData) {
        if (!dom._cmFooterLinksGroup) {
            ...
            dom._cmFooterLeft = createNode(DIV_TAG);
            dom._cmFooterRight = createNode(DIV_TAG);
            ...
            addClassCm(dom._cmFooterLeft, 'footer-left');
            addClassCm(dom._cmFooterRight, 'footer-right');

            appendChild(dom._cmFooterLinksGroup, dom._cmFooterLeft);
            appendChild(dom._cmFooterLinksGroup, dom._cmFooterRight);
        ...
        // Set footer content in left div
        dom._cmFooterLeft.innerHTML = footerData;
        
        // Add Consent ID in right div
        const consentId = getConsentId();
        if (consentId && consentModalData.consentIdLabel) {
            dom._cmFooterRight.innerHTML = `<span class="cc-consent-id">${consentModalData.consentIdLabel}: ${consentId}</span>`;
        }
    }
